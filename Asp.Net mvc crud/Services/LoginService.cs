...
namespace Asp.net_student.Services
{
    ...
    public class LoginService: LoginRepo<Users>
    {
        ...
        {
            Users user = null;
            if (_loginContext.users.Where(e => e.Email== users.Email && e.Name==users.Name ).Any())
            {
                user = _loginContext.users.FirstOrDefault(e => e.Email== users.Email && e.Name==e.Name && e.Password == users.Password);
                return true;
            }
